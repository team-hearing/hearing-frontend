...
import { worldHistoryData as staticWorldHistory, type YearWorldHistory } from "./(components)/worldHistoryData";
import Navigation from "../(components)/Navigation";
import TimelineTitle from "./(components)/TimelineTitle";
import useHorizontalScroll from "./hooks/useHorizontalScroll";
import MobileTimeline from "./(components)/MobileTimeline";
import { api, mapEventsByYearToTimelineData, mapWorldEventsByYear } from "@/lib/api/client";
...
export default function Timeline() {
  ...
  const [worldHistory, setWorldHistory] = useState<YearWorldHistory[]>(staticWorldHistory);
  ...
  // BE에서 한국사 + 세계사 병렬 fetch (실패 시 정적 데이터 유지)
  useEffect(() => {
    let cancelled = false;
    Promise.allSettled([api.getEventsByYear(), api.getWorldEventsByYear()])
      .then(([koResult, worldResult]) => {
        if (cancelled) return;
        if (koResult.status === "fulfilled") {
          const adapted = mapEventsByYearToTimelineData(koResult.value);
          if (adapted.length > 0) {
          ...
          console.warn("[timeline] korean BE fetch failed, fallback:", koResult.reason?.message);
        }
        if (worldResult.status === "fulfilled") {
          const adaptedWorld = mapWorldEventsByYear(worldResult.value);
          if (adaptedWorld.length > 0) setWorldHistory(adaptedWorld);
        } else {
          console.warn("[timeline] world BE fetch failed, fallback:", worldResult.reason?.message);
        }
      });
    return () => {
      cancelled = true;
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-white">
      ...
      <div className="md:hidden timeline-scroll overflow-y-auto flex-grow">
        <MobileTimeline data={timelineData} worldHistory={worldHistory} onYearChange={handleMobileYearChange} />
      </div>
            ...
            <div className="flex gap-32 sm:gap-40 md:gap-52 lg:gap-60 min-w-max relative z-10">
              {timelineData.map((block) => (
                <div key={block.year} data-year={block.year}>
                  <TimelineBlock data={block} worldHistory={worldHistory} />
                </div>
              ))}
            </div>
          </div>
